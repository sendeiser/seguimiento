...
import { 
  GraduationCap, Printer, X, CheckCircle2, Award, Heart, Flame, 
  Calendar, Star, MessageSquareQuote, Pencil, Check, Clock, AlertCircle, FileText
} from "lucide-react";
      ...
      <div className="bg-white rounded-[32px] w-full max-w-2xl overflow-hidden shadow-2xl animate-in zoom-in duration-300 relative border border-slate-200 my-8">
        ...
        <div ref={printRef} className="p-8 space-y-8 bg-white print:p-0 print:space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className={`p-4 rounded-2xl ${currentAtt.bg} border ${currentAtt.border} text-center`}>
              <span className={`text-[10px] font-black uppercase tracking-widest ${currentAtt.text} block`}>Asistencia de Clase</span>
              <span className={`font-['Outfit'] font-black text-xl ${currentAtt.text} mt-2 block flex items-center justify-center gap-1.5`}>
                <AttIcon className="w-5 h-5" /> {currentAtt.label}
              </span>
            </div>
